...
const Dashboard: React.FC<DashboardProps> = ({ entries, hourlyWage }) => {
  ...
  return (
    <div className="space-y-6">
      {/* Zentrale Steuerung */}
      <div className="bg-white p-4 rounded-[2rem] border shadow-sm flex flex-col lg:flex-row items-center justify-between gap-4 no-print sticky top-20 z-30">
        <div className="flex bg-gray-100 p-1 rounded-2xl w-full lg:w-auto">
          <button onClick={() => setActiveSubTab('times')} className={`flex-1 lg:flex-none flex items-center justify-center px-6 py-2.5 rounded-xl font-black text-xs transition-all ${activeSubTab === 'times' ? 'bg-white text-brand-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>
            <Clock className="w-4 h-4 mr-2" /> Zeiten
          </button>
          <button onClick={() => setActiveSubTab('finances')} className={`flex-1 lg:flex-none flex items-center justify-center px-6 py-2.5 rounded-xl font-black text-xs transition-all ${activeSubTab === 'finances' ? 'bg-white text-emerald-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>
        ...
        <div className="flex flex-wrap items-center justify-center gap-3 w-full lg:w-auto">
          <div className="flex items-center space-x-2 bg-gray-50 px-4 py-2 rounded-xl border border-gray-100">
            ...
            <select value={viewMonth} onChange={(e) => setViewMonth(parseInt(e.target.value))} className="bg-transparent font-bold text-sm outline-none cursor-pointer">
              {Array.from({ length: 12 }, (_, i) => (
                <option key={i} value={i}>{getMonthName(i)}</option>
              ))}
            </select>
            <span className="text-gray-300">/</span>
            <select value={viewYear} onChange={(e) => setViewYear(parseInt(e.target.value))} className="bg-transparent font-bold text-sm outline-none cursor-pointer">
          ...
          <button 
            onClick={handleReset}
            className={`p-2.5 rounded-xl transition-all flex items-center group ${isCurrentView ? 'bg-brand-50 text-brand-600' : 'bg-gray-100 text-gray-500 hover:bg-brand-50 hover:text-brand-600'}`}
      ...
      {activeSubTab === 'times' ? (
        <div className="space-y-8 animate-in fade-in duration-500">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2 bg-white p-8 rounded-[3rem] border shadow-sm">
              <h3 className="text-sm font-black text-gray-400 uppercase tracking-[0.2em] mb-8">Stunden-Entwicklung {viewYear}</h3>
            ...
            <div className="space-y-6">
              <div className={`p-8 rounded-[3rem] border transition-all duration-700 ${cursumStats.isOver ? 'bg-red-50 border-red-200 shadow-xl shadow-red-100' : 'bg-white border-gray-100 shadow-sm'}`}>
                ...
                <div className="mb-4">
                  <p className={`text-xs font-bold mb-1 ${cursumStats.isOver ? 'text-red-600' : 'text-gray-400'}`}>
                    {cursumStats.isOver ? 'Budget überschritten!' : 'Verbleibend'}
                  </p>
                  <div className="flex items-baseline space-x-2">
                    <span className={`text-5xl font-black tracking-tighter ${cursumStats.isOver ? 'text-red-600' : 'text-brand-500'}`}>{cursumStats.remaining.toFixed(1)}</span>
              ...
              <div className="bg-white p-8 rounded-[3rem] border shadow-sm">
                <h3 className="text-sm font-black text-gray-400 uppercase tracking-[0.2em] mb-6">Split {getMonthName(viewMonth)}</h3>
                <div className="h-48">
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
        ...
        <div className="space-y-8 animate-in fade-in duration-500">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2 bg-emerald-600 p-10 rounded-[3rem] text-white shadow-2xl shadow-emerald-100 relative overflow-hidden group">
              <div className="absolute top-0 right-0 w-64 h-64 bg-white/10 rounded-full -mr-32 -mt-32 blur-3xl"></div>
              <div className="relative z-10">
                  <p className="text-xs font-black uppercase tracking-[0.3em] opacity-70 mb-2">Netto Einnahmen {getMonthName(viewMonth)} {viewYear}</p>
                  <h2 className="text-6xl font-black tracking-tighter mb-4">{formatCurrency(stats.netMonth)}</h2>
            ...
            <div className={`bg-white p-10 rounded-[3rem] border shadow-xl flex flex-col justify-center relative overflow-hidden group transition-all duration-700 ${!isCurrentView ? 'blur-md opacity-30 grayscale pointer-events-none scale-95' : ''}`}>
               {!isCurrentView && (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div className="lg:col-span-2 bg-white p-8 rounded-[3rem] border shadow-sm">
              <h3 className="text-sm font-black text-gray-400 uppercase tracking-[0.2em] mb-8">Einnahmen-Verlauf {getMonthName(viewMonth)}</h3>
            ...
            <div className="bg-gray-50 p-8 rounded-[3rem] border border-gray-100 space-y-8 h-fit">
               <div className="space-y-6">
...
const PensumCard = ({ label, percent, hours, capacity, isOutOfContext }: { label: string, percent: number, hours: number, capacity: number, isOutOfContext?: boolean }) => {
  ...
  return (
    <div className={`p-6 rounded-[2rem] border shadow-sm transition-all duration-700 relative overflow-hidden group hover:shadow-md ${colorClass} ${isOutOfContext ? 'blur-lg opacity-30 grayscale pointer-events-none scale-95' : ''}`}>
      {isOutOfContext && (
...
const StatCard = ({ title, value, pensum, subText, icon: Icon, color, isOutOfContext }: { 
  ...
  return (
    <div className={`bg-white p-6 rounded-[2.5rem] border shadow-sm group hover:shadow-md transition-all duration-700 relative overflow-hidden ${isOutOfContext ? 'blur-lg opacity-30 grayscale pointer-events-none scale-95' : ''}`}>
      {isOutOfContext && (
