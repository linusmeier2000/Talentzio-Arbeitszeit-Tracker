  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row bg-gray-50 print:bg-white print:block">
      ...
      <main className="flex-1 flex flex-col min-w-0 print:block print:w-full print:static">
        <header className="bg-white/80 backdrop-blur-md border-b h-16 flex items-center px-4 md:px-8 sticky top-0 z-40 justify-between no-print">
           <h2 className="text-sm font-black text-gray-900 uppercase tracking-widest">
             {menuItems.find(m => m.id === activeTab)?.label}
           </h2>
           <div className="md:hidden font-black text-brand-500 tracking-tighter">AT PRO</div>
        </header>
        ...
        {/* Navigation - Mobile */}
        <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white/90 backdrop-blur-lg border-t flex justify-around p-3 z-50 no-print">
          {menuItems.map((item) => (
            <button
              key={item.id}
              onClick={() => setActiveTab(item.id)}
